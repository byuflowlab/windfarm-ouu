...
def getPoints(prob, method_dict, n):

    method = method_dict['method']

    if method == 'dakota':
        # Update dakota file with desired number of sample points
        updateDakotaFile(prob, method_dict['dakota_filename'], n)
        # run Dakota file to get the points locations
        points = getSamplePoints(prob, method_dict['dakota_filename'])
...
def run(prob, impl):
    ...
    for n in range(5,6,1):
        ...
        points = getPoints(prob, method_dict, n)
        ...
        mpi_print(prob, 'Locations at which power is evaluated')
        mpi_print(prob, '\twindspeed \t winddirection')
        for i in range(n):
            mpi_print(prob, i+1, '\t', '%.2f' % windspeeds[i], '\t', '%.2f' % winddirections[i])

        # Set up problem, define the turbine locations and all that stuff
        prob = problem_set_up(prob, impl, windspeeds, winddirections, method_dict)
        ...
        mean_data = prob['mean']
        std_data = prob['std']
        mpi_print(prob, 'mean = ', mean_data/1e6, ' GWhrs')
        mpi_print(prob, 'std = ', std_data/1e6, ' GWhrs')
    ...
    if prob.root.comm.rank == 0:  # I don't know if this is necessary, but it makes me feel more comfortable.
...
def plot(prob):
    ...
    mpi_print(prob, a)
    mpi_print(prob, type(a))
    mpi_print(prob, a.keys())
    mpi_print(prob, json.dumps(a, indent=2))
...
def problem_set_up(prob, impl, windspeeds, winddirections, method_dict=None):
    ...
    # mpi_print(prob, turbineX)
    # mpi_print(prob, turbineY)
    # a = '['
    # for x in turbineX:
    #     a = a + '%.0f' % x + ', '
    # mpi_print(prob, 'turbineX', a)
    # a = '['
    # for y in turbineY:
    #     a = a + '%.0f' % y + ', '
    # mpi_print(prob, 'turbineY', a)
    ...
    prob = Problem(AEPGroup(nTurbines=nTurbs, nDirections=winddirections.size,
                            method_dict=method_dict), impl=impl)
...
if __name__ == "__main__":
    ...
    from openmdao.api import ParallelGroup

    # Trick initialization so I can do the parallel
    method_dict = {}
    method_dict['method'] = 'rect'
    method_dict['uncertain_var'] = 'direction'
    prob = Problem(AEPGroup(nTurbines=1, nDirections=4,
                            method_dict=method_dict), impl=impl)

    # prob = Problem(ParallelGroup(), impl=impl)
    prob.setup(check=False)

    # print(prob.root.comm.rank)
