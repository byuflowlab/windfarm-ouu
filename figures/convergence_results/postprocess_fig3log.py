...
for i, lay in enumerate(layout):
    ...
    for j, m in enumerate(method, 1):
        ...
        ax[i][1].plot(s, mu_err, linewidth=2)
...
for i, lay in enumerate(layout):
    ...
    ax[i][2].set_ylim([0.009, 100.5])
    ax[i][2].set_xlim([-1, 26])
...
# ax[0][3].set_title('number of wind speeds and directions')
