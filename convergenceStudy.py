...
if __name__ == "__main__":
    ...
    AEP_data = np.array([])
    number_bins = np.array([])
    max_min = np.array([])
    for bins in range(1, 50):
        # Define flow properties
        offset = 10.
        windDirections = np.linspace(360/(2*bins)+offset, (360+360/(2*bins)+offset)-360/bins, bins)
       #windFrequencies = bin_frequency(bins)
        f = wind_frequency_funcion()
        windFrequencies = frequ(bins, f)
        ...
        number_bins = np.append(number_bins, bins)
    ...
    plt.ylabel('AEP')
    plt.title('AEP as a function of the Number of Wind Directions')
    ...
    plt.title('Error as a function of the Number of Wind Directions')
    plt.xlabel('Number of Wind Directions')
