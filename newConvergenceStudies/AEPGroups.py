...
class AEPGroup(Group):
    ...
    def __init__(self, nTurbines, resolution=0, nDirections=1, use_rotor_components=False, datasize=0, dakotaFileName=''):
        ...
        self.add('p3', IndepVarComp('weights', np.zeros(nDirections)), promotes=['*'])
        self.add('p4', IndepVarComp('frequency', np.zeros(nDirections)), promotes=['*'])


        self.add('p5', IndepVarComp('turbineX', np.zeros(nTurbines), units=turbine_units), promotes=['*'])
        self.add('p6', IndepVarComp('turbineY', np.zeros(nTurbines), units=turbine_units), promotes=['*'])

        # add vars to be seen by MPI and gradient calculations
        self.add('p7', IndepVarComp('rotorDiameter', np.zeros(nTurbines), units=turbine_units), promotes=['*'])
        self.add('p8', IndepVarComp('axialInduction', np.zeros(nTurbines)), promotes=['*'])
        self.add('p9', IndepVarComp('generator_efficiency', np.zeros(nTurbines)), promotes=['*'])
        self.add('p10', IndepVarComp('air_density', val=1.1716, units='kg/(m*m*m)'), promotes=['*'])
        ...
        if not use_rotor_components:
            self.add('p11', IndepVarComp('Ct_in', np.zeros(nTurbines)), promotes=['*'])
            self.add('p12', IndepVarComp('Cp_in', np.zeros(nTurbines)), promotes=['*'])
        ...
        # Specify how the AEP is computed
