...
from distributions import get_distribution, trapezoid, rectangle
...
if __name__ == "__main__":
    ...
    # I need to define the distribution
    wind_rose_dist = get_distribution()
    print wind_rose_dist

    for n in range(5,6,1):

        # I need to get the points and weights
        # points, weights = cp.generate_quadrature(order=n-1, domain=wind_rose_dist, rule="Clenshaw")
        # points, weights = trapezoid(n-1)
        points, weights = rectangle(n)
        ...
        for i in range(n):
            print i+1,'\t', windspeeds[i], '\t', '%.2f' %winddirections[i]
