...
if __name__=="__main__":
    ...
    for i in range(len(data)):
        ...
        y[i] = data[i][1]/1e6
    print np.max(y)
    print np.max(AEPdirection)
    print y[len(y)-1]
    print AEPdirection[len(AEPdirection)-1]
    plt.plot(x[1:100], AEPdirection[1:], 'g', label='Polynomial Chaos')
    plt.plot(x[1:100], y[1:100], label='Trapezoidal Integration')
    plt.xlim([0,100])
