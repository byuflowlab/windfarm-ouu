...
class AEPGroup(Group):
    ...
    def __init__(self, nTurbines, nDirections=1, method_dict=None):
        ...
        self.add('dv5', IndepVarComp('powers', np.zeros(nDirections), units=power_units), promotes=['*'])
        self.add('dv6', IndepVarComp('dpower_dturbX', np.zeros([nTurbines, nDirections]), units='kW/m'), promotes=['*'])
        self.add('dv7', IndepVarComp('dpower_dturbY', np.zeros([nTurbines, nDirections]), units='kW/m'), promotes=['*'])
